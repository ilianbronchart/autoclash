        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            for text_rect in param['text_rects']:
                ...
                if rect.x <= x <= rect.x + rect.w and rect.y <= y <= rect.y + rect.h:
                    ...
                    cv2.waitKey(1)  # Add a small delay to allow the window to be created


    
    def draw(self, font_scale: float = 0.5, font_thickness: int = 1 , text_color: tuple = (0, 255, 255), padding: int = 5):
        img = self.img.copy()
        ...
        for text_rect in self.text_rects:
            ...
            pt2 = (text_rect.rect.x + text_rect.rect.w, text_rect.rect.y + text_rect.rect.h)
            cv2.rectangle(img, pt1, pt2, (0, 0, 0), -1)

            # Get the text size
            (text_width, text_height) = cv2.getTextSize(text_rect.text, font, font_scale, font_thickness)
            ...
            # Draw text
            cv2.putText(img, text_rect.text, (x, y), font, font_scale, text_color, font_thickness, cv2.LINE_AA)

        cv2.imshow('Image', img)
        cv2.setMouseCallback('Image', self.on_mouse_click, {'text_rects': self.text_rects})
        cv2.waitKey(0)
...
def rects_to_text(img, rects: List[Rect], model: Model) -> OCRResult:
    ...
    with ThreadPoolExecutor() as executor:
        ...
        futures = {executor.submit(process_rect, rect, gray, model): rect for rect in rects}
        
        # Using tqdm to show a progress bar
        for future in tqdm(as_completed(futures), total=len(rects), desc="Processing Rectangles", unit="rect"):
            rect = futures[future]
            try:
